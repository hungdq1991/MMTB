...
{
    public partial class Form_M0006_Detail_P : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        #region Biến
        ...
        private void Initialization_Control()
        {
            ...
            else
            {
                ...
                SetValue_Combobox(cbx_Tailstock);
                SetValue_Combobox(cbx_AxisC);
                TableLength_L = Convert.ToInt32(dataRow[9]);
                TableLength_W = Convert.ToInt32(dataRow[10]);
                Speed = Convert.ToInt32(dataRow[11]);
                BTSize = Convert.ToDecimal(dataRow[12]);
                TipQty = Convert.ToInt32(dataRow[13]);
                WaterLine = Convert.ToBoolean(dataRow[14]);
                AxisXYZ = Convert.ToBoolean(dataRow[15]);
                AxisA = Convert.ToBoolean(dataRow[16]);
                AxisB = Convert.ToBoolean(dataRow[17]);
                AxisC_P = Convert.ToBoolean(dataRow[18]);
                inActive_P = Convert.ToBoolean(dataRow[19]);
            }
        }
